<?php
	...
	class BlockRenderer implements BlockRendererInterface {
		...
		public function renderBlock(BlockView $view, $blockName, array $variables = array()) {
			...
			if (!$resource) {
				throw new LogicException(sprintf('No block "%s" found while rendering the block.', $blockName));
			}
		...
		public function searchAndRenderBlock(BlockView $view, $blockNameSuffix, array $variables = array()) {
			...
			if (!$resource) {
				if (count($blockNameHierarchy) !== count(array_unique($blockNameHierarchy))) {
					throw new LogicException(sprintf('Unable to render the block because the block names array contains duplicates: "%s".', implode('", "', array_reverse($blockNameHierarchy))));
				}

				throw new LogicException(sprintf('Unable to render the block as none of the following blocks exist: "%s".', implode('", "', array_reverse($blockNameHierarchy))));
			}
